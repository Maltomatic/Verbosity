...
for line in lines:
    ...
    if(line.strip() != ""):
        ...
        linecnt += 1
        continue
    src_doc = nlp(line.strip(string.punctuation))
    ...
    for w in src_doc:
        if (w.pos_ != "NUM" and w.pos_ != "PUNCT" and w.pos_ != "SPACE" and w.pos_ != "SYM" and w.text not in contractions):
            doc.append(w)
        # else:
        #     print("removed " + w.text + " " + w.pos_)
    p_space = False
    p_lbr = False
    if(len(doc) > 1 and (doc[-2].pos_ == "CCONJ" or doc[-2].pos_ == "SCONJ")):
        p_lbr = True
    if(len(doc) >= 1 and (doc[-1].pos_ == "CCONJ" or doc[-1].pos_ == "SCONJ")):
        p_space = True
    ...
    if (line_length == 0):
        ...
        if(doc[0].pos_ in ["NOUN", "PRON", "PROPN"]): # nouns for math
        ...
        elif(doc[0].pos_ == "ADV" or ((doc[0].pos_ == "VERB" or doc[0].pos_ == "AUX" or doc[0].pos_ == "ADJ") and in_else == True)): #adverbs for goto
            add_tag(linecnt, in_else)
        ...
        elif(doc[0].pos_ == "VERB" or doc[0].pos_ == "AUX" and in_else == False): # verbs for if/else
            add_tag(linecnt, in_else)
            ln = tmp_l.split()
            ...
            rem = ' '.join(ln[2:])
            if(src_l[0:4] == "    " or src_l[0] == "\t"):
                if(doc[2].pos_ == "VERB" or doc[2].pos_ == "AUX"):
                    poem_code.write(f"if {find_var_tag(indicator, linecnt)} <= 0:\n\t\tgoto .line{summation(rem)%total_ln}\n")
                else:
                    dest = find_var_tag(rem, linecnt)
                    poem_code.write(f"if {find_var_tag(indicator, linecnt)} <= 0:\n\t\texec(f\"goto .line\u007b{dest}\u007d\")\n")
